...
WATCH_OS_VERSION="$(cat Package.swift | grep '.watchOS(.v' | sed -E "s/ *\.watchOS\(\.v([0-9_]*)\),?/\1/g" | sed -E "s/_/./g")"
...
git commit $COMMIT_OPTION -m 'switch back to develop mode' Package.swift
if [ $DEBUG -eq 0 ]; then
    git push origin main
fi
